...
Генератор возвращает итератор, у которого тип — `object`.
...
У стрелочной функции нет собственного контекста.
Значение this в стрелочной функции не зависит от того, как она вызвана, а берется из контекста, в котором функция была
создана.
`this` в стрелочной функции — это `this`, унаследованный из внешнего лексического окружения.
В данном случае — глобальный контекст, где `this` не содержит `x`.

## 32. Какой будет результат?
...
Объяснение:
`new foo()` использует текущее значение `foo.prototype` в момент создания объекта.
Переназначение `foo.prototype` позже никак не влияет на уже созданный объект.
...
* A) undefined и undefined
* B) undefined и { n: 2 }
* C) { n: 2 } и undefined
* D) Ошибка

Ответ: **B)** undefined и { n: 2 }

Объяснение:

Тонкость в порядке присваивания:
`a.x = a = { n: 2 }` - сначала вычисляется `a.x`, но потом a переназначается.
`b` все еще указывает на старый объект `{n: 1}`, у которого поле `x` лежит `{n:2}`.

## 34. Какой будет результат?
...
Функция `parseInt` в JavaScript используется для преобразования строки в целое число.

Принимает два аргумента:

* Первый аргумент — это строка
* Второй аргумент — это основание системы счисления (по умолчанию стоит 10)

Если функция `parseInt` не может преобразовать строку в число, тогда возвращает `NaN` (Not-a-Number).

`map(parseInt)` — передаёт 2 аргумента: value и index.
`parseInt(value, index)` может вести себя неожиданно:

```js
parseInt(1, 0); // 1
parseInt(2, 1); // NaN (основание 1 невалидно)
parseInt(3, 2); // NaN (3 не цифра в двоичной системе)
```
...
Поднятие и инициализация значения переменной `a`.

```js
let a = 1;
```

Внутри функции происходит поиск в родительских лексических окружениях поднятой переменной `a`. После нахождения
выполняет оператор инкремент. `return` выполняет завершение функции.
...
```js
writable: false // установлен флаг false, значение нельзя изменять.
```

## 37. Что делает этот код?
...
```js
b = 5; // глобальная переменная (в нестрогом режиме)
var a = b; // локальная переменная
```

## 38. Какой будет вывод?
...
Оператор `!` преобразует значение в Boolean и делает его отрицание.
...
`setTimeout(obj.logMessage, 1000);` — функция вызывается без контекста, `this` теряется, становится `undefined`.
...
Любой объект, включая `new Boolean(false)`, — `truthy`, даже если внутри `false`.